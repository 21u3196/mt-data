...
<div class="flex-1 flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8 bg-zinc-50">
    <div class="w-full max-w-md bg-white rounded-2xl p-8 sm:p-10 shadow-sm border border-zinc-200">
        ...
        <div class="text-center mb-8">
            ...
            <h1 class="font-heading text-2xl font-bold text-zinc-900 tracking-tight">Welcome Back</h1>
            <p class="text-xs sm:text-sm text-zinc-500 mt-1">Sign in with password or 1-click Face ID</p>
        </div>

        <?php echo $message; ?>

        <!-- Tab Switcher -->
        <div class="p-1 bg-zinc-100 rounded-xl flex gap-1 mb-6 border border-zinc-200">
            <button type="button" id="tabPasswordBtn" onclick="switchAuthTab('password')" class="flex-1 py-2 px-3 rounded-lg text-xs sm:text-sm font-bold transition-all bg-zinc-900 text-white shadow-xs flex items-center justify-center gap-2">
                <i class="fa-solid fa-key text-zinc-300"></i> Password
            </button>
            <button type="button" id="tabFaceBtn" onclick="switchAuthTab('face')" class="flex-1 py-2 px-3 rounded-lg text-xs sm:text-sm font-bold transition-all text-zinc-500 hover:text-zinc-900 flex items-center justify-center gap-2">
            ...
            <form method="POST" autocomplete="on" class="space-y-4">
                <div>
                    ...
                    <div class="relative rounded-xl border border-zinc-300 bg-white focus-within:ring-2 focus-within:ring-zinc-900 focus-within:border-zinc-900 transition-all">
                        ...
                        <input type="email" id="loginEmail" name="email" class="w-full pl-10 pr-4 py-2.5 rounded-xl text-sm text-zinc-900 placeholder-zinc-400 bg-transparent border-0 focus:outline-none focus:ring-0" placeholder="name@example.com" required>
                    </div>
                </div>

                <div>
                    <label class="block text-xs font-bold text-zinc-700 uppercase tracking-wider mb-1.5" for="loginPassword">
                        Password
                    </label>
                    <div class="relative rounded-xl border border-zinc-300 bg-white focus-within:ring-2 focus-within:ring-zinc-900 focus-within:border-zinc-900 transition-all">
                        <span class="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none text-zinc-400 text-sm">
                        ...
                        <input type="password" id="loginPassword" name="password" class="w-full pl-10 pr-11 py-2.5 rounded-xl text-sm text-zinc-900 placeholder-zinc-400 bg-transparent border-0 focus:outline-none focus:ring-0" placeholder="••••••••" required>
        ...
        <div id="faceAuthPane" style="display: none;" class="space-y-4 text-center">
            <div class="relative w-52 h-52 sm:w-56 sm:h-56 mx-auto rounded-2xl overflow-hidden border-2 border-zinc-900 bg-black flex items-center justify-center shadow-inner group">
                <video id="faceVideo" class="w-full h-full object-cover scale-x-[-1]" autoplay playsinline muted></video>
                
                <!-- Reticle Overlay -->
                <div id="scanReticle" class="absolute inset-4 rounded-2xl border border-white/40 pointer-events-none transition-all"></div>
                
                <!-- Laser Sweep Line -->
                <div class="absolute left-0 right-0 h-0.5 bg-emerald-400 shadow-[0_0_8px_#34d399] animate-laser pointer-events-none"></div>
            </div>

            <div id="scanStatus" class="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full text-xs font-bold bg-zinc-100 text-zinc-800 border border-zinc-200">
                <span class="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></span>
                <span>Starting camera...</span>
            </div>

            <div>
                <div class="relative rounded-xl border border-zinc-300 bg-white mb-3 text-left">
                    <span class="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none text-zinc-400 text-sm">
                        <i class="fa-solid fa-at"></i>
                    </span>
                    <input type="email" id="biometricTargetEmail" class="w-full pl-10 pr-4 py-2 rounded-xl text-xs sm:text-sm text-zinc-900 placeholder-zinc-400 bg-transparent border-0 focus:outline-none focus:ring-0" placeholder="Optional: Target email for instant match">
                </div>

                <button type="button" onclick="triggerFaceScan()" class="w-full py-2.5 px-4 rounded-xl bg-zinc-900 hover:bg-zinc-800 text-white font-bold text-sm shadow-sm transition-all flex items-center justify-center gap-2">
